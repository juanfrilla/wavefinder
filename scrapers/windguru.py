...
class Windguru(object):
    ...
    def format_forecast(self, forecast: Dict, tides: dict) -> Dict:
        ...
        times = [dt.split(".")[1].replace("h", ":00") for dt in forecast["datetime"]]
        datetimes = generate_datetimes(dates, times)
        forecast["date"] = [dt.date() for dt in datetimes]
        forecast["time"] = [dt.time() for dt in datetimes]
        ...
        forecast["spot_name"] = generate_spot_names(forecast)
        forecast["date_friendly"] = [
            datetime_to_frontend_str(moment) for moment in datetimes
        ]
        time_friendly = [moment.strftime("%H:%M") for moment in datetimes]
        forecast["time_friendly"] = time_friendly
        forecast["time_graph"] = [
            element.replace(":", r"\:") for element in time_friendly
        ]
